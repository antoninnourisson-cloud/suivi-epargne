  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-fade-in" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-6" onClick={e => e.stopPropagation()}>
        ...
        {state.message && <p className="text-sm text-slate-500 mb-4">{state.message}</p>}
        {state.kind === 'prompt' && (
          <input
            autoFocus
            value={value}
            ...
            className="w-full p-3 mb-4 bg-slate-50 border border-slate-200 rounded-xl font-bold text-slate-800 focus:ring-2 focus:ring-indigo-500 outline-none"
          />
        )}
        <div className="flex gap-2 justify-end">
          <button onClick={onClose} className="px-4 py-2 rounded-xl font-bold text-sm text-slate-600 hover:bg-slate-100">Annuler</button>
          <button
            onClick={confirm}
