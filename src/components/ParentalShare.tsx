  ...
  return (
    <div className="space-y-6 animate-fade-in pb-20">
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        <h2 className="text-2xl font-black text-slate-800 flex items-center gap-2 mb-1"><Users className="w-6 h-6 text-indigo-600" /> Part parentale</h2>
        <p className="text-sm text-slate-500">Distingue ton capital réel de l'argent géré pour tes parents.</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-white border border-slate-200 p-6 rounded-2xl">
          <p className="text-slate-400 text-xs font-bold uppercase mb-1 flex items-center gap-2"><User className="w-4 h-4 text-indigo-600" /> Mon capital</p>
          <p className="text-4xl font-black text-indigo-600">{fmt(totalOwned)}</p>
        </div>
        <div className="bg-white border border-slate-200 p-6 rounded-2xl">
          <p className="text-slate-400 text-xs font-bold uppercase mb-1 flex items-center gap-2"><Users className="w-4 h-4 text-amber-500" /> Capital parents</p>
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        ...
        <p className="text-[11px] text-slate-400 mt-2">Total géré : {fmt(grand)}</p>
      </div>
        ...
        <table className="w-full text-left text-sm">
          <thead className="bg-slate-50 border-b border-slate-200">
            <tr>
              ...
              <th className="px-6 py-3 text-[10px] font-black text-slate-400 uppercase text-right">Moi</th>
              <th className="px-6 py-3 text-[10px] font-black text-slate-400 uppercase text-right">Parents</th>
              <th className="px-6 py-3 text-[10px] font-black text-slate-400 uppercase w-1/3">Répartition</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100">
            ...
            {rows.length === 0 && <tr><td colSpan={4} className="px-6 py-8 text-center text-slate-400 italic">Aucun compte à afficher.</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
