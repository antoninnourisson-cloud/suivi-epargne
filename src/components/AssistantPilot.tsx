    ...
    <div className="space-y-8 animate-fade-in pb-20">
        ...
        <>
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-200"><label className="text-[10px] font-black text-slate-400 uppercase">Brut Annuel</label><input type="number" value={Math.round(grossAnnual)} onChange={e => updateFromGrossAnnual(parseFloat(e.target.value)||0)} className="w-full bg-transparent font-black text-slate-800 text-lg outline-none" /></div>
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-200"><label className="text-[10px] font-black text-slate-400 uppercase">Brut Mensuel</label><input type="number" value={Math.round(autoValues.grossMonth)} onChange={e => updateFromGrossMonth(parseFloat(e.target.value)||0)} className="w-full bg-transparent font-black text-slate-800 text-lg outline-none" /></div>
              ...
              <div className="bg-white p-4 rounded-xl text-xs space-y-3 border border-slate-200 animate-in slide-in-from-top-2 shadow-inner mb-4">
                 ...
                 <div className="bg-amber-50 p-2 rounded-lg border border-amber-100">
                    ...
                    <div className="flex items-center justify-between text-[10px] gap-2">
                        <div className="flex flex-col"><span className="text-slate-500">Taux Barème (Auto) : <strong>{autoValues.autoRate.toFixed(1)}%</strong></span>{taxRateManual > 0 && <span className="text-amber-600">Force à : <strong>{taxRateManual}%</strong></span>}</div>
                        <div className="flex items-center gap-1"><label className="text-slate-400">Forcer taux :</label><input type="number" step="0.1" value={taxRateManual} onChange={(e) => setTaxRateManual(parseFloat(e.target.value) || 0)} className="w-12 p-1 text-right bg-white border border-amber-200 rounded font-bold outline-none" placeholder="Auto"/><span className="text-slate-400">%</span></div>
                    </div>
                 </div>
              </div>
            )}
      ...
      {activeTab === 'fiscal' && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
           {fiscalClock.map((item: any) => (<div key={item.id} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm relative overflow-hidden"><div className={`absolute top-0 right-0 p-16 opacity-5 rounded-full -mr-8 -mt-8 ${item.isAvailable ? 'bg-emerald-500' : 'bg-indigo-500'}`}></div><div className="flex justify-between items-start mb-4"><div className={`p-3 rounded-xl ${item.isAvailable ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-500'}`}>{item.isAvailable ? <Unlock className="w-6 h-6" /> : <Lock className="w-6 h-6" />}</div><span className="text-[10px] font-black uppercase bg-slate-100 px-2 py-1 rounded text-slate-500">{item.type}</span></div><h4 className="font-bold text-slate-800 text-lg mb-1">{item.name}</h4><div className="border-t border-slate-100 pt-4 mt-4"><div className="flex justify-between items-end"><div><p className="text-[10px] text-slate-400 uppercase font-bold">Échéance</p><p className="font-bold text-slate-700">{item.date}</p></div><div className={`text-right font-black text-xl ${item.isAvailable ? 'text-emerald-500' : 'text-indigo-600'}`}>{item.timeLeft}</div></div></div></div>))}
        </div>
      )}
    </div>
  );
