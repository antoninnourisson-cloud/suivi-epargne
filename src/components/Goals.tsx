...
export const Goals: React.FC<GoalsProps> = ({ goals, onUpdateGoals, expenses, income, fiscalConfig, workBenefits }) => {
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-20">
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col sm:flex-row justify-between gap-4 sm:items-center">
        <div>
          <h2 className="text-2xl font-black text-slate-800 flex items-center gap-2"><Target className="w-6 h-6 text-indigo-600" /> Objectifs d'épargne</h2>
      ...
      {isAdding && (
        <div className="bg-white p-6 rounded-2xl border border-indigo-200 shadow-sm space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div><label className="text-[10px] font-black text-slate-400 uppercase">Nom</label><input autoFocus value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="Ex : Épargne de précaution" className="w-full p-2 bg-slate-50 border border-slate-200 rounded-lg font-bold" /></div>
            <div><label className="text-[10px] font-black text-slate-400 uppercase">Montant cible (€)</label><input type="number" value={form.targetAmount} onChange={e => setForm({ ...form, targetAmount: e.target.value })} className="w-full p-2 bg-slate-50 border border-slate-200 rounded-lg font-bold" /></div>
            <div><label className="text-[10px] font-black text-slate-400 uppercase">Déjà épargné (€)</label><input type="number" value={form.savedAmount} onChange={e => setForm({ ...form, savedAmount: e.target.value })} className="w-full p-2 bg-slate-50 border border-slate-200 rounded-lg font-bold" /></div>
            <div><label className="text-[10px] font-black text-slate-400 uppercase">Échéance (optionnel)</label><input type="date" value={form.deadline} onChange={e => setForm({ ...form, deadline: e.target.value })} className="w-full p-2 bg-slate-50 border border-slate-200 rounded-lg font-bold" /></div>
          </div>
          <div className="flex gap-2 justify-end">
            <button onClick={resetForm} className="px-4 py-2 rounded-xl font-bold text-sm text-slate-600 hover:bg-slate-100 flex items-center gap-1"><X className="w-4 h-4" /> Annuler</button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {goals.map(g => {
          ...
          return (
            <div key={g.id} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
              <div className="flex justify-between items-start mb-3">
                <div>
                  <h3 className="font-black text-slate-800 text-lg">{g.name}</h3>
                  <p className="text-xs text-slate-500">{fmt(g.savedAmount)} / {fmt(g.targetAmount)}</p>
                </div>
              ...
              {!done && (
                <div className="flex flex-wrap gap-3 text-xs">
                  <span className="inline-flex items-center gap-1 text-slate-600 bg-slate-50 px-3 py-1.5 rounded-lg font-bold">
              ...
              <div className="mt-4 pt-4 border-t border-slate-100 flex items-center gap-2">
                <label className="text-[10px] font-black text-slate-400 uppercase">Mettre à jour l'épargne</label>
                <input
                  type="number"
                  defaultValue={g.savedAmount}
                  onBlur={e => patchGoal(g.id, { savedAmount: parseFloat(e.target.value) || 0 })}
                  className="w-28 p-1.5 bg-slate-50 border border-slate-200 rounded-lg font-bold text-sm"
                />
